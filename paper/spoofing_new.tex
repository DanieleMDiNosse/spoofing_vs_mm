...
the visible limit order book and consolidates related executions into clusters. A cluster can be anchored either to the
participant's resting order being executed or to the participant trading aggressively. We attribute orders through the
original client code when it is available and otherwise through the firm identifier, while preserving the identity level.
The strict gate is a four-condition conjunction: same-actor liquidity posted on the side opposite to an execution during the
preceding $90$ seconds is withdrawn within $2$ seconds; the withdrawn quantity exceeds the clustered execution quantity; the
price first moves in a direction favorable to the execution; and it subsequently reverts. An empirical top-$10$ depth kernel
weights visible liquidity by book rank. Across three Italian equity samples, the reconstruction finds rapid withdrawals after
both passive and aggressive executions, while the complete four-condition sequence selects a smaller subset. These findings
support reproducible surveillance triage, but they do
not constitute a statistical test, identify manipulative intent, estimate causal effects, or measure the prevalence of
spoofing. Legitimate liquidity provision remains an explicit alternative explanation.
\end{abstract}
...
participant-level messages without treating frequent cancellation itself as evidence of spoofing. The proposed screen
therefore combines five observable elements: a recent opposite-side liquidity profile, an execution by the same actor, rapid
withdrawal of that profile, a price movement favorable to the execution, and subsequent price reversion. The execution can
be passive or aggressive. Actor identity is a separate data problem, so we use the original client code when present and the
firm identifier only as a fallback. We also consolidate related execution messages into clusters and assign each physical
cancellation to at most one cluster.
...
The full four-condition sequence is less common because it additionally requires withdrawn quantity to exceed clustered
...
not a statistical validation, a label of manipulation, or an estimate of causal price impact.
...
linking these observed actions through anonymized actor identifiers at the client or firm level.
...
The two namespaces are kept distinct, and every output records whether attribution is at the client or firm level. Messages
missing both identifiers are excluded from actor-attributed analysis; none occurs in the reported samples. Firm fallback is
coarser than client attribution and can combine several clients represented by the same firm. It is therefore an identity
fallback, not evidence that those clients are one economic account.

Execution role is defined independently of actor identity. We retain fills marked exclusively passive or exclusively
aggressive and reject fills for which the two role flags are both set or both absent. Related messages are consolidated into
one execution cluster when they share the trading partition, canonical actor, order identifier, execution role, and side.
Passive clusters also require a common resting price; aggressive clusters can span several traded prices under the same
order and sweep. Consecutive fills must be no more than $100$ milliseconds apart, with no intervening lifecycle boundary.
The $100$-millisecond threshold is an operational choice, and the reported findings are conditional on it; sensitivity to
alternative inter-fill gaps is not reported here. Raw-message identifiers and cluster memberships remain available as
provenance, while execution clusters form the analytical unit for event metrics and actor--partition aggregation. The
...
central question is whether a recent liquidity profile disappears specifically after the same participant executes on the
opposite side. To answer it, we first represent the participant's liquidity across the top $n$ book levels. We then measure
the conditional withdrawal of that profile and test whether the associated price path is economically consistent with the
hypothesized strategy. The withdrawal and price-response quantities remain separate because they describe different parts
of the mechanism.
...
child messages are clustered; it does not change actor identity. In both routes, the candidate posture is sought among the
...
weighted sides are absent. Setting $n=1$ gives the best-level measure. Larger values of $n$ also capture candidate liquidity
...
Let $c$ denote an execution cluster for canonical actor $i$. Its ordered child executions share the trading partition,
actor key, order identifier, execution role, and side, and they satisfy the operational inter-fill-gap rule. Passive child
executions also share the resting price. Aggressive child executions use reported traded price and quantity and may span
several price levels within the same order or sweep. The timestamps $t_c^{\mathrm{start}}$ and $t_c^{\mathrm{end}}$ denote
the first and last child executions. The quantity $Q_c$ is their total executed quantity, and $s_c\in\{b,a\}$ is the actor's
execution side. We denote the opposite candidate-posture side by $\bar{s}_c$. The pre- and post-cluster observation times are
\begin{equation}
...
same-actor top-$n$ orders first observed inside this pre-execution window. This restriction separates a recent candidate
...
Let $\mathcal{A}^{i,s}_{c,k,\Theta}(t)$ contain the active orders of actor $i$ on side $s$ and rank $k\leq n$ at time
...
in \cref{eq:relative_agent_depth}. Write
$T^{i}_{c,n,\Theta}(t)=L^{i,a}_{c,n,\Theta}(t)+L^{i,b}_{c,n,\Theta}(t)$ for the corresponding total weighted liquidity. We
define the episode-conditioned imbalance piecewise as
\begin{equation}
    ...
    0,
    & T^{i}_{c,n,\Theta}(t)=0
      \text{ and the zero-profile state is retained},\\
    \text{undefined}, & \text{otherwise}.
    \end{cases}
    \label{eq:episode_conditioned_dwi}
\end{equation}
This statistic describes the recent liquidity layer associated with cluster $c$, rather than the actor's entire standing
book. Active profiles with a zero weighted denominator remain undefined rather than being converted into finite evidence.
All subsequent collapse and withdrawal diagnostics use this episode-conditioned profile.
...
weighted liquidity that disappears from each side after the fill using an exact piecewise definition:
\begin{equation}
    C^{i,s}_{c,n}
    =\begin{cases}
    \dfrac{
        ...
        L^{i,s}_{c,n,\Theta}(t_c^{-})
    },
    & L^{i,s}_{c,n,\Theta}(t_c^{-})>0,\\
    0,
    & L^{i,s}_{c,n,\Theta}(t_c^{-})=0.
    \end{cases}
    \qquad s\in\{b,a\}.
...
change can instead generate material collapse on both sides. The boundary values have a direct interpretation:
...
cancellation more than once, let $\mathcal E(o)$ contain the eligible same-actor, opposite-side clusters that precede
...
\end{equation}
where $Q_c>0$ is the execution-cluster quantity by construction. Let $Q^{i,\bar{s}_c}_{c,n}$ denote the visible quantity of
the candidate profile before execution. We report the fraction of that profile removed as
\begin{equation}
    A^{i}_{c,n}
    =\begin{cases}
...
\end{equation}
The two ratios answer different questions. $R^{i}_{c,n}$ compares the assigned withdrawal with the executed quantity, whereas
$A^{i}_{c,n}$ compares it with the candidate profile that was available to withdraw. These quantities remain separately
inspectable. The full descriptive gate requires $U^{i}_{c,n}>Q_c$ rather than applying a threshold to a composite score. We
summarize repeated behavior through counts and withdrawal rates at the actor--partition level. We do not use a scalar
aggregate to rank actors or infer intent.
...
\begin{equation}
    h_c=
    \begin{cases}
        +1, & \text{if the clustered execution is a sell execution,}\\
        -1, & \text{if the clustered execution is a buy execution.}
    \end{cases}
...
against the pre-execution favorable direction after the assigned cancellation or cancellations. For quantity-weighted cluster
...
We compute each diagnostic with both the mid-price and the queue microprice in the event-level records. The reported summary
tables and the event gate use the mid-price versions. We report price responses separately from withdrawal counts,
...
\begin{enumerate}
    \item Resolve the canonical actor and classify each fill as exclusively passive or aggressive. Construct cluster $c$
    from compatible child executions while preserving the ordered mapping to raw messages.
    ...
    \item Report assigned withdrawal quantity, withdrawal-to-execution ratio, and removed-profile fraction.
    ...
    \item Summarize repeated behavior with cluster counts and actor-level recurrence diagnostics. Retain raw child-execution
...
withdrawal, withdrawn quantity greater than cluster execution quantity, positive favorable pre-execution mid-price movement,
and positive mid-price reversion from the assigned cancellation over an observable $2$-second horizon. The conjunction is
deliberately stricter than withdrawal matching alone. Right-censored reversion outcomes fail the gate because the final
condition cannot be observed. This transparent rule supports analyst triage; it is descriptive and is not a statistical
test, manipulation label, or proof of intent.
...
Rapid same-actor withdrawals occur in all three samples and after both passive and aggressive executions. The complete
withdrawal-and-price sequence is less frequent because it also requires the withdrawn quantity, favorable movement, and
post-cancellation reversion conditions. \Cref{tab:empirical_spoofing_diagnostics} reports this progressive narrowing under
the same episode-conditioned top-$10$ reconstruction. Each instrument uses its full-sample empirical kernel from
\cref{tab:empirical_depth_kernel_calibration} with $H=10$ seconds.

A matched cluster is a passive or aggressive execution followed within $2$ seconds by a canonically assigned cancellation
of a same-actor, opposite-side candidate order. The separate $10$-second interval is the structural reconstruction horizon.
The table distinguishes clusters from their raw child execution messages and counts each assigned physical cancellation
once. The firm-fallback column records the coarser cases in which the original client code is missing. The strict column then
retains matched clusters that also satisfy the withdrawal-size and two price-response conditions. These quantities are
descriptive screening diagnostics, not manipulation labels or evidence of intent.
...
reconstruction identifies \FerrariMatchedClusters{} matched clusters for Ferrari, \NexiMatchedClusters{} for Nexi, and
...
being used as a substitute for the full hypothesized price path.
...
The instrument-specific counts in \Cref{tab:empirical_spoofing_diagnostics} preserve the provenance of these conditional
shares. The Risanamento sample contains \RisanamentoRawFills{} raw execution messages, consolidated into
\RisanamentoClusters{} clusters: \RisanamentoPassiveClusters{} passive and \RisanamentoAggressiveClusters{} aggressive.
Among the \RisanamentoMatchedClusters{} matched clusters, \RisanamentoPassiveMatchedClusters{} have passive anchors and
\RisanamentoAggressiveMatchedClusters{} have aggressive anchors; \RisanamentoFirmFallbackMatchedClusters{} use firm fallback
and jointly own \RisanamentoAssignedCancellations{} assigned physical cancellations. Nexi contains \NexiRawFills{} raw
execution messages and \NexiClusters{} clusters, split into \NexiPassiveClusters{} passive and \NexiAggressiveClusters{}
aggressive anchors. Its \NexiMatchedClusters{} matched clusters comprise \NexiPassiveMatchedClusters{} passive and
\NexiAggressiveMatchedClusters{} aggressive anchors; \NexiFirmFallbackMatchedClusters{} use firm fallback and jointly own
\NexiAssignedCancellations{} assigned cancellations. Ferrari contains \FerrariRawFills{} raw execution messages and
\FerrariClusters{} clusters, of which \FerrariPassiveClusters{} are passive and \FerrariAggressiveClusters{} are aggressive.
Its matched set contains \FerrariPassiveMatchedClusters{} passive and \FerrariAggressiveMatchedClusters{} aggressive anchors;
\FerrariFirmFallbackMatchedClusters{} use firm fallback and jointly own \FerrariAssignedCancellations{} assigned cancellations.

\subsection{Risanamento}
\label{subsec:risanamento_results}

Risanamento has the smallest matched set, and only \RisanamentoCompatibleSequences{} clusters satisfy the strict
four-condition gate. Favorable pre-execution movement is nevertheless positive in \RisanamentoFPMShare{} of matched clusters
with an observed diagnostic, while positive cancel-anchored reversion occurs in \RisanamentoREVShare{}. The relatively low
reversion share shows that many matched withdrawals fail one price-path condition; the strict gate also depends on withdrawal
size and favorable pre-execution movement.
...
Reconstruction identifies \NexiMatchedClusters{} matched clusters, of which \NexiCompatibleSequences{} pass the strict gate.
Positive favorable pre-execution movement occurs in \NexiFPMShare{} of matched clusters with an observed diagnostic, while
...
Reconstruction identifies \FerrariMatchedClusters{} matched clusters, and \FerrariCompatibleSequences{} pass the strict
gate. Among observed matched-cluster diagnostics, the shares with positive favorable pre-execution movement and positive
...
in identity resolution, matched-cluster count, withdrawal scale, and price-path composition. \Cref{tab:top_actor_results}
therefore reports the three largest groups in each instrument by matched-cluster count rather than collapsing them into one
...
positive mid-price favorable movement and reversion.
...
required withdrawal size, favorable pre-execution movement, and post-cancellation reversion. This gap is scientifically
important because it prevents a high cancellation rate or a large withdrawal from standing in for the full mechanism.

The mathematical construction makes the separation explicit. The quantities $U^{i}_{c,n}$, $R^{i}_{c,n}$, and
$A^{i}_{c,n}$ describe what liquidity disappeared and how its scale compares with the execution and candidate profile. The
quantities $\operatorname{FPM}^{i}_{c}$ and $\operatorname{REV}^{i}_{c}$ describe the associated price path. Requiring both
types of evidence produces the strict gate, but the conjunction remains a descriptive compatibility rule. It neither
estimates spoofing prevalence nor establishes that any retained sequence was manipulative.

The strongest limitation is the absence of a counterfactual comparison. The analysis does not test whether withdrawal is
more likely after the focal actor's execution than after an otherwise comparable non-event, and repeated observations from
the same actors or trading periods need not be independent. The depth kernels are also calibrated on the same full samples
...
$100$-millisecond child-execution clustering threshold.
...
remains an alternative explanation. The three instrument samples cover different periods and actor compositions, which rules
out causal cross-instrument comparisons.
