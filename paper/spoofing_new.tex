...
the visible limit order book and groups related execution messages into an execution cluster. A cluster is anchored either by the
actor's passive resting-order execution or by the actor's aggressive execution. We attribute orders through the original
client code when it is available and otherwise through the firm identifier, while preserving the identity level. For each execution cluster, the candidate set comprises the same actor's opposite-side orders that are active in the top $10$ ranks immediately
before the first execution and were first observed no more than $90$ seconds earlier. Clusters of the same actor, side, trading
partition, and date that share a candidate-order lifecycle form one candidate-posture episode. The strict gate passes only when four
conditions hold jointly at episode level: at least one unique candidate-order cancellation is canonically assigned within $2$ seconds; attributed
withdrawal exceeds total episode execution quantity; observed favorable post-publication midprice movement is positive; and
observed cancel-anchored midprice reversion is positive over a $2$-second horizon. An empirical depth kernel weights visible
liquidity by book rank. In the Risanamento, Nexi, and Ferrari samples, respectively, the reconstruction identifies
\RisanamentoMatchedClusters{}, \NexiMatchedClusters{}, and \FerrariMatchedClusters{} matched execution clusters, while
\RisanamentoCompatibleSequences{}, \NexiCompatibleSequences{}, and \FerrariCompatibleSequences{} distinct episodes satisfy the strict gate.
...
\begin{figure}[htbp]
    ...
    \caption{Rate at which each actor cancels opposite-side liquidity within two seconds of its own execution. For actor
    $i$, the denominator $N_i^E$ contains all of its passive and aggressive executions. The numerator counts executions
    followed by at least one cancellation by the same canonical actor on the opposite side during
    ...
    are ranked from highest to lowest rate within each identity scope and instrument. The actor-rank axis is logarithmic, and no actor
    identifier is displayed. Identity follows the client-first rule: the original client code is used when available, with
    the firm identifier used only as a labelled fallback. Marker area increases with $\log(1+N_i^E)$ to make sample support
    visible; marker shape and color also distinguish the two identity scopes. No actor is removed for low
...
participant-level messages without treating frequent cancellation itself as evidence of spoofing. Complete top-$10$
resting-profile measures first describe the shape of an actor's displayed liquidity; these diagnostics do not determine
eligibility. For each passive or aggressive execution cluster, we then fix a candidate set of same-actor, opposite-side
orders that are active in the top $10$ ranks and no more than $90$ seconds old. A matched cluster has at least one cancellation
of a candidate order canonically assigned within $2$ seconds. Clusters connected through a shared candidate-order lifecycle
form one candidate-posture episode. The strict four-condition gate additionally requires the episode's unique attributed
withdrawal to exceed its total execution quantity, observed favorable post-publication midprice movement to be
positive, and observed cancel-anchored midprice reversion to be positive over the $2$-second reversion horizon. Actor identity
...
physical cancellation is assigned to at most one episode.

The empirical reconstruction identifies rapid same-actor withdrawals in all three samples and after both execution routes.
...
withdrawal can motivate review, but it is not equivalent to a sequence that also satisfies the size and price-path conditions
...
Withdrawal Multilevel Spoofing Conditionality Index (WMSCI) is the primary event-prioritization metric. It ranks events by the
...
sequence. All four strict overlaps occur where the external firm-level subject maps to multiple canonical actors, so they
establish subject-scope coverage rather than exact-actor correspondence. Because the external alerts are positive seeds
rather than independently adjudicated labels, this comparison reports recovery relative to the source alerts. Exact-actor
strict recall is defined only for intervals whose external and internal identities have aligned granularity; it is not recall
...
the midprice changes, as adverse-selection risk changes, or as inventory moves away from its target. The dealer generally
maintains a two-sided presence near the top of the book within this stylized benchmark, although actual quoting can become
...
this profile as evidence of short-term price pressure and adjust their orders. If other traders respond to the displayed
...
After the smaller order executes, rapid withdrawal of the displayed opposite-side profile is consistent with reducing
...
quote updates: the candidate withdrawal follows the opposite-side execution rather than only changes in fair value,
inventory, or adverse-selection risk. \Cref{tab:surveillance-models} summarizes these stylized incentives; none is directly
observed as intent.
...
methods move the unit of analysis from isolated orders to networks of traders and transactions. This representation can
...
the model below preserves the auditability of rule-based surveillance while producing descriptive participant-level order
sequences from proprietary event data.
...
This procedure yields $34$ distinct timed intervals, while the date-only Risanamento record remains outside the event-level
denominator.

For timed records, we parse the source bounds at millisecond precision and include both endpoints. To place raw events on a
common event-time axis, we use the first nonmissing timestamp in the source-field priority order \texttt{TRADETIME},
\texttt{BOOKOUTTIME}, \texttt{BOOKIN}, and \texttt{SEQUENCETIME}. We recover an execution when at least one of its
constituent execution messages lies inside the closed union interval. Matched-withdrawal status is read from the cluster
diagnostics, whereas strict status is deduplicated by candidate-posture episode. The union interval, rather than each potentially overlapping source record, is therefore
...
several canonical actor keys when client identifiers are available internally. We therefore distinguish subject-scope
overlap from exact-actor correspondence and treat the latter as identifiable only when the external identity maps to one
canonical actor at the same granularity. The alert report contains no negative sample, so the comparison cannot estimate a
false-positive rate.

For each instrument, we reconstruct the visible order book from order-level events and infer the tick size from the
reconstructed best quotes. We compute actor-wide resting-profile diagnostics over the first ten book ranks and separately
construct an age-conditioned set of opposite-side candidate orders. The main specification uses a $10$-second structural
post-execution window, a $2$-second rapid-withdrawal attribution window, a $2$-second reversion horizon measured from each
assigned cancellation, and a maximum candidate-order age of $90$ seconds.
...
attribution is at the client or firm level. A client-derived key and a firm-fallback key are distinct canonical identities;
the fallback does not merge a client-identified message with a firm-only message. Messages missing both identifiers are
excluded from actor-attributed analysis; none occurs in the reported samples. Firm fallback is coarser than client
...
$100$ milliseconds apart, with no intervening cancellation, modification, or new-order lifecycle event for the same order.
The $100$-millisecond threshold is an operational choice, and the reported findings are conditional on it; sensitivity to
alternative inter-execution-message gaps is not reported here. Raw-message identifiers and the constituent messages assigned to each execution cluster remain available as
provenance. Cluster-level quantities remain diagnostic; connected clusters that share a candidate-order lifecycle are evaluated
jointly as one candidate-posture episode, and actor--day recurrence counts distinct episodes. The
...
book levels, which supplies descriptive DWI, SCI, and MSCI shape diagnostics. The second is an age-conditioned set of
opposite-side candidate orders, which supplies matched-withdrawal quantities and the event-prioritization metric WMSCI.
Separately, the strict gate applies the prespecified withdrawal-size, favorable-movement, and reversion conditions. These
sign and size restrictions are screening conditions, not estimates of causal influence or intent. The resting-profile,
withdrawal, and price-response quantities remain separate because they describe different parts of the mechanism.

The motivating rate in \cref{fig:actor_cancellation_rank} stops at the broad temporal association between an execution and a
later opposite-side cancellation. The reconstruction below narrows this population in stages. It first requires an
attributable recent candidate withdrawal, then compares the withdrawn and executed quantities, and finally checks the
favorable-movement and reversion conditions. The broad rate does not enter these later gates.
...
\begin{enumerate}
    ...
    liquidity profile on one side of the book, typically away from the touch. The actor has an opposite-side execution
    anchor, which may be passive or aggressive in the operational reconstruction.
...
These three states illustrate the hypothesized mechanism; their qualitative descriptors do not define strict-set
membership. For either execution route, the candidate set consists of same-actor, opposite-side orders observed in the top
$10$ ranks before an execution cluster and satisfying the $90$-second age limit. A cancellation within $2$ seconds supplies rapid
matched-withdrawal evidence. Clusters sharing a candidate posture are evaluated jointly; the strict gate then requires unique
attributed withdrawal to exceed total episode execution quantity and both
midprice response diagnostics to be positive. The detector observes these conditions, not the participant's intent or the
profile's causal effect on other traders. The recency restriction prevents long-lived liquidity from entering a candidate
...
\end{equation}
where $D^{s}_{k}(t)$ is the total displayed market depth on side $s$ and rank $k$. The zero-depth branch makes the
rank-level convention explicit; an imbalance remains undefined when the participant has no positive weighted footprint on
either side.
...
the displayed price. A bid rank is reached when the minimum bid execution price is at most the displayed price. We estimate the
rank-level message-snapshot hit frequency, denoted $\widehat p^{m,s}_{k}(H)$, from pooled hit and exposure counts:
\begin{equation}
    \widehat p^{m,s}_{k}(H)
    =
    \frac{
...
a parametric fit. Its probability interpretation is conditional on the message-snapshot sampling scheme. The estimator is a
short-horizon proxy for whether the displayed price is reached, not an individual order's execution probability, which also
...
floor, relative weights can be driven primarily by protection and must not be interpreted as evidence of rank-specific price
association. Every reported kernel in \cref{tab:empirical_depth_kernel_calibration} passes the normalization condition and
...
weighted sides are absent. The reconstructed state series additionally records a one-time zero-profile transition when a
...
actor key, order identifier, execution role, and side, and they satisfy the operational inter-execution-message-gap rule. Passive executions
...
For event prioritization, we also summarize the scale and speed of the attributed withdrawal. Define the delay-weighted
withdrawal quantity
\begin{equation}
    \widetilde U^{i}_{c,n}
    =
...
Under the piecewise definition, WMSCI is zero when no positive candidate-layer or delay-weighted attributed-withdrawal
contribution is available; this does not imply that the unrestricted resting profile was unchanged. WMSCI is non-negative
and has no fixed upper bound. We use it as the primary metric for ordering event-level candidates for
...
$U^{i}_{c,n}>Q_c$ and the separate price-path conditions; it does not apply a WMSCI threshold. At the actor--partition level,
...
The matched-withdrawal variables show whether an actor removes same-actor opposite-side liquidity after its own
...
both quotes and positive top-level depth are available, the two market-price proxies are the midprice
...
pre-cancellation and horizon values of $M$ are observed and $w_{c,o}>0$ is finite. The execution-level post-cancellation
reversion is the quantity-and-delay weighted mean
\begin{equation}
...
against the pre-execution favorable direction after the assigned cancellation or cancellations. For quantity-weighted execution
...
We compute each diagnostic with both the midprice and the queue microprice in the event-level records. The reported summary
...
price response from episodes that also exhibit favorable post-publication movement and post-cancellation reversion.
...
\begin{enumerate}
    \item Resolve the canonical actor and classify each execution message as exclusively passive or aggressive. Construct execution cluster $c$
    ...
    $\operatorname{DWI}^{i}_{n}(t_c^{-})$.
    \item From the opposite-side top-$n$ orders in this pre-execution state, retain the fixed candidate set
    ...
    \item Compute SCI, side-specific collapse, and MSCI from the unrestricted resting-profile states at $t_c^{-}$ and
    $t_c^{+}$.
    \item Report cluster-level assigned withdrawal quantity, withdrawal-to-execution ratio, removed-profile fraction, and WMSCI. Use WMSCI
    to order cluster diagnostics for human review without applying it as a strict-gate threshold.
    \item Compute signed price diagnostics from the first candidate-layer posting, through the execution, to the
    post-cancellation window.
    \item Within one actor, execution side, trading partition, and recorded event date, connect clusters that share at least one
    candidate-order lifecycle. Each connected component is one candidate-posture episode $e$; passive and aggressive cluster
    membership remains explicit, and a physical cancellation contributes once.
    \item Summarize repeated behavior with distinct episode counts at actor--day level. Retain clusters and their raw constituent
    execution messages as provenance.
\end{enumerate}
This procedure is designed to distinguish ordinary book repricing from withdrawal of participant-attributed candidate
liquidity. Canonical assignment and episode-level deduplication ensure that one physical cancellation does not inflate the metrics of
several neighboring clusters under the stated eligibility rule.

For episode $e$, let $\mathcal{C}_e$ be its cluster set, $Q_e=\sum_{c\in\mathcal{C}_e}Q_c$ its total execution quantity,
and $U_e$ the sum of unique attributed physical cancellations. We aggregate $\operatorname{FPM}_e$ across all member clusters
with execution-quantity weights and $\operatorname{REV}_e$ across unique withdrawals with the stated quantity-and-delay
weights. These episode diagnostics are available only when every required member observation is covered. The
\textit{spoofing-compatible sequence} gate passes only when four conditions hold simultaneously:
(i) at least one eligible opposite-side cancellation is canonically assigned within $2$ seconds of a member execution;
(ii) $U_e>Q_e>0$; (iii) the observed midprice $\operatorname{FPM}_e$ is strictly positive; and (iv) the observed
cancel-anchored midprice $\operatorname{REV}_e$ is strictly positive. A missing or right-censored price diagnostic
...
The broad rate in \cref{fig:actor_cancellation_rank} counts any qualifying opposite-side cancellation after an execution.
The results below narrow that behavior to cancellations of the fixed, age-conditioned candidate set and assign each physical
cancellation canonically. These matched same-actor withdrawals occur in all three samples and after both passive and
aggressive execution clusters. Clusters sharing a candidate-order lifecycle are then evaluated jointly. The complete
strict episode is necessarily a smaller subset because, in addition to rapid canonical matching, it requires
$U_e>Q_e$, positive favorable post-publication movement, and positive cancel-anchored reversion.
\Cref{tab:empirical_spoofing_diagnostics} reports this narrowing under the same actor-wide top-$10$ state and
age-conditioned candidate-order reconstruction. Each instrument uses its full-sample empirical kernel from
...
The attributed cancellation quantity cannot exceed the candidate order's visible quantity before execution, while the raw
cancellation-time quantity remains available for audit. The table distinguishes executions from their raw constituent execution
messages and counts each assigned physical cancellation once. The firm-fallback column records the coarser cases in which the
original client code is missing. The strict column reports episode participation in each anchor branch; mixed-anchor episodes therefore appear in both branch
rows but are counted once in instrument-level strict totals. These quantities are descriptive screening diagnostics, not
manipulation labels or evidence of intent.
...
The main empirical pattern is a gap between mechanical matching and the complete price-consistent episode. The
reconstruction identifies \FerrariMatchedClusters{} matched execution clusters for Ferrari, \NexiMatchedClusters{} for Nexi, and
\RisanamentoMatchedClusters{} for Risanamento. The strict gate retains \FerrariCompatibleSequences{},
\NexiCompatibleSequences{}, and \RisanamentoCompatibleSequences{} distinct episodes, respectively. Favorable
post-publication movement is
positive in \FerrariFPMShare{}, \NexiFPMShare{}, and \RisanamentoFPMShare{} of observed matched clusters, whereas positive
reversion occurs in \FerrariREVShare{}, \NexiREVShare{}, and \RisanamentoREVShare{}. Thus, the withdrawal condition is not
being used as a substitute for the full hypothesized price path. These are marginal shares among observed matched-execution
diagnostics and do not decompose episode-level strict-set attrition across the jointly required conditions.
...
\Cref{tab:empirical_spoofing_diagnostics} reports raw-execution provenance, execution-cluster composition, firm-fallback counts,
and uniquely assigned physical cancellations for each instrument. Cluster rows provide branch-specific diagnostics; distinct
candidate-posture episodes are the strict detection and actor--day recurrence unit.

\subsection{Risanamento}
\label{subsec:risanamento_results}

Risanamento has the smallest matched set, and only \RisanamentoCompatibleSequences{} distinct episodes satisfy the strict
four-condition gate. Favorable post-publication movement is nevertheless positive in \RisanamentoFPMShare{} of matched clusters
with an observed diagnostic. Among matched clusters with an observed reversion diagnostic, \RisanamentoREVShare{} have
positive cancel-anchored reversion. The strict gate additionally requires positive favorable post-publication movement and
unique attributed withdrawal exceeding total episode execution quantity, so these marginal shares do not identify which requirement excludes any
particular matched episode.

For each of the \RisanamentoMatchedClusters{} matched clusters, the review record preserves an episode-first dossier and a
reconstructed top-$10$ event window. These records organize observed facts, counter-evidence, and legitimate alternatives.
They are triage aids, not empirical labels or proof of manipulation.

\subsection{Nexi}
\label{subsec:nexi_results}

Nexi exhibits matched withdrawals after both execution routes, but the full price-consistent path remains a subset.
Reconstruction identifies \NexiMatchedClusters{} matched execution clusters, while \NexiCompatibleSequences{} distinct episodes pass the strict gate.
Positive favorable post-publication movement occurs in \NexiFPMShare{} of matched clusters with an observed diagnostic, while
positive cancel-anchored reversion occurs in \NexiREVShare{}.
...
Reconstruction identifies \FerrariMatchedClusters{} matched execution clusters, and \FerrariCompatibleSequences{} distinct episodes pass the strict
gate. Among observed matched-cluster diagnostics, the shares with positive favorable post-publication movement and positive
...
Recurrence is informative only when actor identity and execution role remain explicit. The largest actor--anchor groups differ
in identity resolution, matched-execution count, withdrawal scale, and price-path composition. \Cref{tab:top_actor_results}
therefore reports the three largest groups in each instrument by matched-execution count rather than collapsing them into one
...
the largest matched withdrawal-to-execution ratio within the group; FPM$+$ and REV$+$ are the shares of matched clusters with
...
can combine several clients, a firm-level count has coarser attribution than a client-level count and should not be
interpreted as a single-account history.

The primary empirical object is the actor-attributed association between execution and rapid opposite-side withdrawal, with
client-level attribution when an original client code is observed and coarser firm-level attribution otherwise. We report
its magnitude and repetition descriptively. WMSCI orders event-level candidates for review; MSCI remains a complete-resting-
profile context diagnostic; neither metric is a strict-gate threshold or an actor-ranking score. An analyst must also
...
At the externally reported subject scope, the comparison recovers an execution in every distinct timed interval,
...
the closed union interval. Withdrawal columns count intervals containing at least one matched execution cluster, while strict-scope columns count intervals
containing at least one strict candidate-posture episode. Strict subject scope requires at least one episode in the interval to satisfy all four
gate conditions. The final column reports strict overlaps among intervals whose external
identity maps to one canonical actor at the same granularity; entries report strict intervals divided by identity-aligned
intervals. The date-only Risanamento record is excluded from all timed denominators.}
...
All four strict subject-scope overlaps occur within the $30$ identity-unaligned Ferrari intervals for which the main externally
reported firm maps to two canonical actor keys. Exact-actor correspondence is therefore not identifiable in those intervals. By contrast,
the comparison contains four identity-aligned intervals---three for Ferrari and one for Nexi---and none contains a strict
...
executions, including one execution with an attributed withdrawal and no strict sequence. These full-day counts establish that the
subject is observable, but they cannot identify correspondence with the specific external event.

These results measure recovery relative to externally reported activity. Exact-actor strict recall is identifiable only for
the four identity-aligned unioned timed intervals, of which none contains a strict sequence. The four strict overlaps among
the remaining intervals are subject-scope overlaps, not exact-actor recall. Because the source is positive-only and was not
independently re-adjudicated for manipulative intent, these quantities do not estimate recall or precision against true
...
Matched episodes occur after both passive and aggressive execution clusters in all three samples. Only a subset also exhibits the
required withdrawal size, favorable post-publication movement, and post-cancellation reversion. This distinction matters
because it prevents a high cancellation rate or a large withdrawal from standing in for the full mechanism.

The external-alert coverage audit reinforces this separation. The reconstruction covers execution in all $34$ distinct timed
...
$A^{i}_{c,n}$ describe how much of the pre-execution candidate layer can be attributed to the withdrawal and how its scale
...
WMSCI separately prioritizes event-level review by combining candidate-layer scale, rapid attributed withdrawal, and the
removed-profile fraction. Unlike MSCI, it is not a complete-resting-profile collapse measure; unlike the strict gate, it is
not a pass--fail condition.
MSCI is a separate resting-profile diagnostic: it uses all active same-actor top-$n$ orders, so long-lived balanced liquidity
...
positive reference periods but no negative controls. They support source-alert recovery and exact-actor strict-recall
accounting under the audit's identity-alignment rule, but they cannot identify recall or precision against independently
adjudicated manipulation labels or calibrate detector thresholds. The depth kernels are also calibrated on the same full
samples to which they are applied, so their displayed-price associations are descriptive rather than out-of-sample evidence.
Right-censored reversion outcomes fail the event gate, and the reported results do not test sensitivity to the
$100$-millisecond execution grouping threshold, the $90$-second candidate age, or the reported $2$- and $10$-second
windows.

Identity and sampling impose further limits. Firm fallback loses within-firm client resolution and can pool several
accounts, so firm-level recurrence is not directly comparable with recurrence under a client code. Aggressive execution is
an execution-role observation, not evidence about the intent of the surrounding resting orders; ordinary liquidity taking
remains an alternative explanation. External firm-level alerts can also span several internal actor keys, which prevents
exact-actor evaluation in those intervals. The three instrument samples cover different periods and actor compositions, which
rules out causal cross-instrument comparisons.
