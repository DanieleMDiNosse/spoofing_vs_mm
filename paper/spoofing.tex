...
We retain the normalized shape change and the two side-specific collapse fractions as separate diagnostics. We do not
multiply them into a composite score. This separation shows whether the change arises from the opposite side, the same
side, or both, and prevents a two-sided quote refresh from becoming a large scalar through multiplication alone. These
quantities are retained as event-level diagnostics and in client-level feature summaries; they do not enter the
four-condition event gate or the generated result tables.
